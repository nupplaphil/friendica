...
- **BREAKING**: Removed deprecated standalone scripts `bin/daemon.php`, `bin/jetstream.php` and `bin/worker.php`. Use the `bin/console.php` subcommand instead.
...
- **BREAKING**: Removed deprecated class `Friendica\Core\Addon`. Use implementation of `Friendica\Core\Addon\AddonHelper` via constructor injection or `\Friendica\DI::addonHelper()` instead.
...
- **BREAKING**: Removed deprecated class `Friendica\Core\Addon\Model\AddonLoader`. Use `\Friendica\Core\Addon\AddonHelper::getAddonDependencyConfig()` instead.

- **BREAKING**: Removed deprecated interface `Friendica\Core\Addon\Capability\ICanLoadAddons`. Use `\Friendica\Core\Addon\AddonHelper` instead.

- **BREAKING**: Removed deprecated class `Friendica\Core\Logger`. Use constructor injection or `Friendica\DI::logger()` instead.
...
- **BREAKING**: Removed deprecated classes `Friendica\Core\Logger\Factory\AbstractLoggerTypeFactory`, `Friendica\Core\Logger\Factory\Logger`, `Friendica\Core\Logger\Factory\StreamLogger` and `Friendica\Core\Logger\Factory\SyslogLogger`. Implement `\Friendica\Core\Logger\Factory\LoggerFactory` instead.
...
- **BREAKING**: Removed support for the deprecated `monolog` value for `system.logger_config`. Use `stream` or `syslog` instead.

- **BREAKING**: Removed deprecated method `Friendica\DI::workerLogger()`. Use `Friendica\DI::logger()` instead.
...
- **BREAKING**: Removed deprecated method `\Friendica\BaseRepository::_selectOne()`. Use `\Friendica\BaseRepository::_selectFirstRowAsArray()` instead.
