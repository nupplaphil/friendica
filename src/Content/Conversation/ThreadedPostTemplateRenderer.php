...
use Friendica\Model\Contact\User as ContactUser;
...
/**
 * Builds threaded post template data from item trees.
 */
class ThreadedPostTemplateRenderer
{
	 ...
	 */
	public function renderThreadRoot(array $item, RenderThreadContext $renderContext, array $convResponses, string $formSecurityToken): ?array
	{
		if (!isset($item['uri-id'], $item['guid'], $item['id'])) {
			return null;
		}

		return $this->buildThreadTemplateData($item, $renderContext, $convResponses, $formSecurityToken, 1, []);
	}
	 ...
	 *
	 * @param array<string, mixed> $item
	 * @param RenderThreadContext $renderContext
	 * @return array{shareable: bool, announceable: bool, commentable: bool, likeable: bool}
	 */
	private function determineActionPermissions(array $item, RenderThreadContext $renderContext): array
	{
		$shareable    = in_array($renderContext->getProfileOwner(), [0, $this->session->getLocalUserId()]) && ($item['private'] ?? ItemModel::PUBLIC) != ItemModel::PRIVATE;
		...
		if ($commentable && $this->session->getLocalUserId() && !empty($item['author-id']) && ContactUser::isIsBlocked($item['author-id'], $this->session->getLocalUserId())) {
			$commentable = false;
		}
	...
	private function buildModerationButtons(array $item): array
	{
		...
		if (empty($item['self']) && $this->session->getLocalUserId()) {
			...
			if ($authorBaseUri->getHost() && !$this->baseURL->isLocalUrl($authorBaseUri)) {
				$ignoreServer = [
	 ...
	 */
	private function buildEventData(array $item, RenderThreadContext $renderContext): array
	{
		$isevent = false;
		$attend  = [];
	 ...
	 */
	private function buildActionButtons(array $item, RenderThreadContext $renderContext, bool $likeable, bool $shareable, bool $announceable): array
	{
		$buttons = [
			'like'     => null,
			'dislike'  => null,
	...
	/**
	 * @param array<string, mixed> $item
	 * @param array<string, mixed> $threadParents
	 * @return array<string, mixed>|null
	 */
	private function buildThreadTemplateData(array $item, RenderThreadContext $renderContext, array $convResponses, string $formSecurityToken, int $threadLevel, array $threadParents): ?array
	{
		...
		$bodyHtml = ItemModel::prepareBody($item, true, $renderContext->isPreview());
		...
		if ($threadLevel === 1 && $this->session->getLocalUserId()) {
			$ignored = PostModel\ThreadUser::getIgnored($item['uri-id'] ?? 0, $this->session->getLocalUserId());
			if ($ignored) {
				$ignore_thread = [
		...
		$connector = !in_array($item['network'] ?? '', Protocol::NATIVE_SUPPORT) && (($item['protocol'] ?? '') != ConversationModel::PARCEL_JETSTREAM)
			? $this->l10n->t('Connector Message')
			: false;

		$permissions  = $this->determineActionPermissions($item, $renderContext);
		...
		$tagger = '';
		if ($this->session->getLocalUserId() && $renderContext->getProfileOwner() == $this->session->getLocalUserId() && !empty($item['uid'])) {
			$tagger = [
		...
		if (!$this->session->getLocalUserId() && ($item['network'] ?? '') !== Protocol::DIASPORA && !empty($this->session->get('remote_comment'))) {
			...
			$comment_html = $this->getCommentBox($item, $indent, $renderContext);
		}
		...
		$tmpItem = [
			...
			'previewing'             => $renderContext->isPreview() ? ' preview ' : '',
		...
		if (!empty($item['children']) && is_array($item['children'])) {
			...
			foreach ($item['children'] as $child) {
				...
				$childData                           = $this->buildThreadTemplateData($child, $renderContext, $convResponses, $formSecurityToken, $threadLevel + 1, $nextThreadParents);
				if ($childData !== null) {
					$children[] = $childData;
				}
			}
			if (($nb_children > 2) || ($threadLevel > 1)) {
	 ...
	 * @return string
	 */
	private function getCommentBox(array $item, string $indent, RenderThreadContext $renderContext): string
	{
		if (!$this->session->getLocalUserId() || !$renderContext->isWritable()) {
			return '';
		}
		...
		$uid = $renderContext->getProfileOwner();
		...
		$template = Renderer::getMarkupTemplate('comment_item.tpl');

		return Renderer::replaceMacros($template, [
			'$default'     => $default,
			'$return_path' => $this->arguments->getQueryString(),
			'$threaded'    => true,
			'$jsreload'    => '',
			'$wall'        => ($renderContext->getMode() === 'profile'),
			'$id'          => $item['id'] ?? 0,
			'$parent'      => $item['id'] ?? 0,
			'$qcomment'    => $qcomment,
			'$profile_uid' => $uid,
	...
	private function getQuoteShares($quoteshares): array
	{
		...
		return ['total' => $quoteshares['total'], 'title' => $this->l10n->t('Quote shared by: %s', implode(', ', $quoteshares['title']))];
	}
}
